			...
			<swiper-item v-for="(listItem,listIndex) in list" :key="listIndex">
				...
				<view class='content'>
					<view class='card' v-for="(item,index) in listItem" v-if="listItem.length > 0" :key="index" @click="updateTo(item.id)">
						标题：{{item.todoAbstract}}
						<view class="uni-media-list-body">
							<view class="uni-media-list-text-bottom">
								<text>协助人：</text><div v-if="item.assistantPerson">{{item.assistantPerson.username}}</div>
								<text>抄送人：</text> <div v-if="item.copyPerson">{{item.copyPerson.username}}</div>
							</view>
